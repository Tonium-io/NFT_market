...
controller_abi = Abi.from_path(
             path='./true-nft/components/true-nft-core/build/uploadDeGenerative.abi.json')
...
with open('./true-nft/components/true-nft-core/build/uploadDeGenerative.tvc', 'rb') as fp:     
...
call_set = CallSet(
            function_name='constructor',
            header=FunctionHeader(pubkey=keypair_NFTowner.public),input=dict(codeIndex=codeIndex.code,codeData=codeData.code,pay=1,koef=120))
encode_params = ParamsOfEncodeMessage(
...
for i in range(5):
    call_set = CallSet(
                ...
                header=FunctionHeader(pubkey=keypair_controller1.public),input=dict(adr=NFTToken_adr,metadata=json.dumps(dict(name="Name")).encode("utf-8").hex()))
    encode_params = ParamsOfEncodeMessage(
...
call_set = CallSet(
            ...
            header=FunctionHeader(pubkey=keypair_controller1.public),input=dict(adr=NFTToken_adr))
encode_params = ParamsOfEncodeMessage(
            abi=controller_abi, signer=Signer.Keys(keypair_controller1),address=controller1,
            call_set=call_set)
process_params = ParamsOfProcessMessage(
